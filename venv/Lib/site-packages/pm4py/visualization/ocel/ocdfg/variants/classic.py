...
def apply(ocdfg: Dict[str, Any], parameters: Optional[Dict[Any, Any]] = None) -> Digraph:
    ...
    for act in act_count:
        if len(act_count[act]) >= act_threshold: # Default implemenation

        # if len(act_count[act]) <= act_threshold: # Customization: less or equal to activity_threshold is kept.
    ...
    for ot in edges_count:
        for act_cou in edges_count[ot]:
            if act_cou[0] in nodes and act_cou[1] in nodes:
                if len(edges_count[ot][act_cou]) >= edge_threshold: # Default implemenation

                # if len(edges_count[ot][act_cou]) <= edge_threshold: # Customization: less or equal to edge_threshold
                    if annotation == "frequency":
    ...
    for ot in sa_count:
        if ot in edges_count:
            for act in sa_count[ot]:
                if act in nodes:
                    if len(sa_count[ot][act]) >= edge_threshold: # Default implemenation
                    # if len(sa_count[ot][act]) <= edge_threshold: # Customization: less or equal to edge_treshold
    ...
    for ot in ea_count:
        if ot in edges_count:
            for act in ea_count[ot]:
                if act in nodes:
                    if len(ea_count[ot][act]) >= edge_threshold: # Default implemenation
                    # if len(ea_count[ot][act]) <= edge_threshold: # Customization: less or equal to edge_treshold
